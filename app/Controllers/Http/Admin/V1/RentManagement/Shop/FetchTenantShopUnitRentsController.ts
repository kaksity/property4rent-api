...
import {
  ...
  SUCCESS,
  TENANT_SHOP_UNIT_RENT_LIST_FETCH_SUCCESSFUL,
  VALIDATION_ERROR,
} from 'App/Helpers/Messages/SystemMessage'
import TenantShopUnitRentActions from 'App/Actions/TenantShopUnitRentActions'
import FetchTenantShopUnitRentsValidator from 'App/Validators/Admin/V1/RentManagement/Shop/FetchTenantShopUnitRentsValidator'
...
export default class FetchTenantShopUnitRentsController {
  ...
  public async handle({ request, response }: HttpContextContract) {
    try {
      try {
        await request.validate(FetchTenantShopUnitRentsValidator)
      ...
      const {
        ...
        landlord_identifier: landlordIdentifier,
      ...
      return response.ok({
        status: SUCCESS,
        status_code: this.ok,
        message: TENANT_SHOP_UNIT_RENT_LIST_FETCH_SUCCESSFUL,
