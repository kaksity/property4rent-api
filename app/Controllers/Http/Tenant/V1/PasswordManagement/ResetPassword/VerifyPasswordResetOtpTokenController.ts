...
import VerifyPasswordResetOtpTokenValidator from 'App/Validators/Tenant/V1/PasswordManagement/ResetPassword/VerifyPasswordResetOtpTokenValidator'
...
export default class VerifyPasswordResetOtpTokenController {
  ...
  public async handle({ request, response }: HttpContextContract) {
    ...
    try {
      ...
      if (otpToken?.authorId !== tenant?.id) {
        await dbTransaction.rollback()
