...
Route.group(function () {
  ...
  ).as('landlord.v1.onboarding.request-account-activation-otp-token').middleware('auth:landlordTeamMember')
  Route.post(
    'account-activation/verify-otp-token',
    'VerifyAccountVerificationOtpTokenController'
  ).as('landlord.v1.onboarding.verify-account-activation-otp-token').middleware('auth:landlordTeamMember')
})
